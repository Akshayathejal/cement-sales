...
# Read raw data
df = pd.read_excel(r"C:\Users\thejalraj\OneDrive\Desktop\Project\data given.xlsx")
df
...
y_data = np.array(df.order_quantity_milliontonne)
...
df.groupby(['year'])['sale_quantity_milliontonne'].agg('sum')
...
data_2018 = df[df['year'] == 2018]['order_quantity_milliontonne']
...
for i in range(2015, 2023):
    ts_data = df[df['year'] == i]['sale_quantity_milliontonne']
    plt.plot(range(len(ts_data)), ts_data, label=i)
plt.ylim((0, y_max))
plt.title('Annual Sale trends', fontsize=16)
plt.xlabel('Month', fontsize=10)
plt.ylabel('sale_quantity_milliontonne', fontsize=10)
...
plt.figure(figsize=(18, 7), dpi=200)
for i in range(2015, 2023):
    ts_data = df[df['year'] == i]['order_quantity_milliontonne']
    plt.plot(range(len(ts_data)), ts_data, label=i)
plt.ylim((0, y_max))
plt.title('Annual Order trends', fontsize=16)
plt.xlabel('Month', fontsize=10)
plt.ylabel('order_quantity_milliontonne', fontsize=10)
plt.xticks(fontsize=10)
plt.legend(loc='lower right')
plt.show()
...
sns.boxplot(x='year', y='sale_quantity_milliontonne', data=df, ax=axes[0])
sns.boxplot(x='month', y='sale_quantity_milliontonne', data=df, ax=axes[1])
...
# Draw Plot
fig, axes = plt.subplots(1, 2, figsize=(20, 8), dpi= 200)
sns.boxplot(x='year', y='order_quantity_milliontonne', data=df, ax=axes[0])
sns.boxplot(x='month', y='order_quantity_milliontonne', data=df, ax=axes[1])
...
# Plot chart
plt.figure(figsize=(18, 7), dpi=200)
plt.plot(x_date, y_data, '-', label='sale_quantity_milliontonne')
...
tsa_plots.plot_acf(df.sale_quantity_milliontonne, lags = 12)


# In[88]:


model = ARIMA(train.sale_quantity_milliontonne, order = (12,1,6))
res = model.fit()
...
# Evaluate forecasts
rmse = sqrt(mean_squared_error(test.sale_quantity_milliontonne, pred))
...
pyplot.plot(test.sale_quantity_milliontonne, color ='blue')
...
auto_model = pm.auto_arima(df['sale_quantity_milliontonne'], trace=True, suppress_warnings=True)
...
model = ARIMA(train.sale_quantity_milliontonne, order = (2,1,2))
res = model.fit()
print(res.summary())
...
test["sale_quantity_milliontonne"].mean()


# In[100]:


model2 = ARIMA(df.sale_quantity_milliontonne, order = (2,1,2))
model2 = model2.fit()
df.tail()
...
pyplot.plot(df.sale_quantity_milliontonne, color ='red')
